...
namespace SocketModule
{
    ...
    public:
        ...
        //4.接收客户端连接（仅TCP需要，返回新的Socket对象处理客户端通信） 
        virtual std::shared_ptr<Socket> Accept(InetAddr *client) = 0; // 返回值：std::shared_ptr<Socket> - 新的Socket对象（智能指针自动管理资源）

        //5.关闭Socket文件描述符（释放文件描述符）
        virtual void Close() = 0;

        //6.接收数据（将数据存入out字符串，返回读取字节数）
        virtual int Recv(std::string *out) = 0;
        //7.发送数据（发送message字符串，返回发送字节数）
        virtual int Send(const std::string &in) = 0;

        //8.连接服务器（指定IP和端口，返回连接结果）
        ...
        void BuildTcpServerSocketMethod(uint16_t port, int backlog = gbacklog)
        {
        ...
        //2.构建TCP客户端Socket的固定流程，仅创建Socket（模板方法模式）
        void BuildTcpClientSocketMethod()
        {
            SocketOrDie();   // 创建Socket（客户端无需绑定/监听）
        }
    ...
    public:
        ...
        //6.接收数据（流式读取，拼接至out字符串）
        int Recv(std::string *out) override
        {
            //1.临时缓冲区（1024字节）
            char buffer[1024];  

            //2.recv：从Socket读取数据，0表示默认读取模式（阻塞）
            ssize_t n = ::recv(_sockfd, buffer, sizeof(buffer) - 1, 0);
            if (n > 0)  // 读取到数据
            {
                buffer[n] = 0;  // 添加字符串结束符（避免乱码）
                *out += buffer; // 将读取的数据拼接至out（处理半包）
            }

            //3.返回读取的字节数（0表示客户端断开，-1表示错误）
            return n; 
        }
